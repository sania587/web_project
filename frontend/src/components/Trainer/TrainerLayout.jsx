...
const TrainerLayout = ({ children }) => {
  ...
  const navLinks = [
    { path: "/trainer/dashboard", label: "Dashboard", icon: FaHome },
    { path: "/profile", label: "My Profile", icon: FaUser },
    { path: "/trainer/workouts", label: "My Workouts", icon: FaDumbbell },
    { path: "/trainer/exercises", label: "Exercise Library", icon: FaRunning },
    { path: "/trainer/diet-plans", label: "Diet Plans", icon: FaAppleAlt },
    { path: "/trainer/schedule", label: "My Schedule", icon: FaCalendarAlt },
    { path: "/trainer/clients", label: "My Clients", icon: FaUserFriends },
    { path: "/trainer/notifications", label: "Notifications", icon: FaBell },
  ];
    ...
    {/* Desktop Toggle */}
            ...
            <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center w-full' : ''}`}>
              ...
              {!isCollapsed && (
                <div>
                  ...
                  <p className="text-xs" style={{ color: theme.colors.textSecondary }}>Trainer Portal</p>
                </div>
              )}
            </div>
            <button 
      ...
      <div className={`flex-1 flex flex-col min-h-screen overflow-x-hidden transition-all duration-300 ${isCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
        ...
        >
          ...
          <span className="font-bold" style={{ color: theme.colors.text }}>Trainer Portal</span>
